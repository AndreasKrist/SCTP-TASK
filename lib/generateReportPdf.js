  ...
  if (user.answers && user.questionIds) {
    ...
    answerSections.forEach(({ key, label }) => {
      ...
      qIds.forEach((qId, idx) => {
        ...
        // Options — use saved shuffle order if available, else original a/b/c/d
        const displayOrder = user.shuffledOptionsMap?.[qId] || OPTION_LABELS;
        displayOrder.forEach((originalKey, displayIndex) => {
          checkPage(7);
          ...
          doc.text(`${displayLabel}.`, margin + 4, y);
